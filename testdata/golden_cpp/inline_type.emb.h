 ...
 public:
  ...
  bool Ok() const {
...

    {
      ...
      if (emboss_reserved_local_field_present.ValueOrDefault() && !status().Ok()) return false;
    }
...

    {
      const auto emboss_reserved_local_field_present = ::emboss::support::Maybe</**/bool>(true);
      if (!emboss_reserved_local_field_present.Known()) return false;
      if (emboss_reserved_local_field_present.ValueOrDefault() && !secondary_status().Ok()) return false;
    }
