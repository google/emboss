...
def _get_fully_qualified_name(name, ir):
  return (
      _get_fully_qualified_namespace(name, ir)
      + "::"
...
class _FieldRenderer(object):
  ...
  def render_field_read_with_context(
      self, expression, ir, prefix, subexpressions
  ):
...
def _generate_structure_virtual_field_methods(
    enclosing_type_name, field_ir, ir
):
...
def _split_enum_case_values_into_spans(enum_case_value):
  ...
  while start <= end:
    ...
    while (
        substr_start < substr_end and enum_case_value[substr_end - 1].isspace()
    ):
      substr_end -= 1
...
def _get_enum_value_names(enum_value):
  ...
  return [
      name_conversion.convert_case("SHOUTY_CASE", case, name) for case in cases
  ]
    ...
    if word in _CPP_RESERVED_WORDS:
      errors.append([
          error.error(
              source_file_name,
              namespace_value.source_location,
              f'Reserved word "{word}" is not allowed as a namespace'
              " component.",
          )
      ])
