...
# Reflection backend.
#
# Mirrors the C++ backend above: `emboss_reflection_library` is a macro that
# creates both an `emboss_library` (IR only) and a `reflection_emboss_library`
# (JSON only), so users can write `emboss_reflection_library(name=..., srcs=...)`
# and get a machine-readable description of their `.emb` at `<name>.emb.json`.

def emboss_reflection_library(name, srcs, deps = [], import_dirs = [], **kwargs):
    """Constructs a JSON reflection library from an .emb file.
    ...
    if len(srcs) != 1:
        fail(
            "Must specify exactly one Emboss source file for " +
            "emboss_reflection_library.",
            "srcs",
        )
    ...
    reflection_emboss_library(
        name = name,
        deps = [":" + name + "_ir"],
        **kwargs
    )

EmbossReflectionInfo = provider(
    fields = {
        "files": "(depset[File]) The `.emb.json` files from this rule.",
        "transitive_files": "(depset[File]) The `.emb.json` files from this " +
                            "rule and all dependencies.",
    },
    doc = "Provides JSON files generated by the Emboss reflection backend.",
)

def _reflection_emboss_aspect_impl(target, ctx):
    emboss_info = target[EmbossInfo]
    src = emboss_info.direct_source
    json_files = [ctx.actions.declare_file(src.basename + ".json", sibling = src)]
    args = ctx.actions.args()
    args.add("--input-file")
    args.add_all(emboss_info.direct_ir)
    args.add("--output-file")
    args.add_all(json_files)
    ctx.actions.run(
        executable = ctx.executable._emboss_reflection_compiler,
        arguments = [args],
        inputs = emboss_info.direct_ir,
        outputs = json_files,
        mnemonic = "EmbossReflection",
    )
    transitive_files = depset(
        direct = json_files,
        transitive = [
            dep[EmbossReflectionInfo].transitive_files
            for dep in ctx.rule.attr.deps
            if EmbossReflectionInfo in dep
        ],
    )
    return [
        EmbossReflectionInfo(
            files = depset(json_files),
            transitive_files = transitive_files,
        ),
    ]

_reflection_emboss_aspect = aspect(
    implementation = _reflection_emboss_aspect_impl,
    attr_aspects = ["deps"],
    required_providers = [EmbossInfo],
    attrs = {
        "_emboss_reflection_compiler": attr.label(
            executable = True,
            cfg = "exec",
            default = "@com_google_emboss//compiler/back_end/reflection:emboss_codegen_reflection",
        ),
    },
)

def _reflection_emboss_library_impl(ctx):
    if len(ctx.attr.deps) != 1:
        fail("`deps` attribute must contain exactly one label.", attr = "deps")
    dep = ctx.attr.deps[0]
    return [
        dep[EmbossInfo],
        dep[EmbossReflectionInfo],
        DefaultInfo(files = dep[EmbossReflectionInfo].files),
    ]

reflection_emboss_library = rule(
    implementation = _reflection_emboss_library_impl,
    attrs = {
        "deps": attr.label_list(
            aspects = [_reflection_emboss_aspect],
            allow_rules = ["emboss_library"],
            allow_files = False,
        ),
    },
    provides = [EmbossInfo, EmbossReflectionInfo],
)
