...
# Wireshark / Lua backend.
#
# Mirrors the C++ backend above: `emboss_lua_library` is a macro that creates
# both an `emboss_library` (IR-only) and a `lua_emboss_library` (Lua source
# only), so users can simply write `emboss_lua_library(name=..., srcs=...)`
# and get a runnable Wireshark dissector at `<name>.emb.lua`.

def emboss_lua_library(name, srcs, deps = [], import_dirs = [], **kwargs):
    """Constructs a Wireshark Lua dissector library from an .emb file.
...
EmbossLuaInfo = provider(
    fields = {
        "files": "(depset[File]) The `.emb.lua` files from this rule.",
        "transitive_files": "(depset[File]) The `.emb.lua` files from this " +
                            "rule and all dependencies.",
    },
    doc = "Provides Lua dissector files generated by the Emboss Wireshark backend.",
)

def _lua_emboss_aspect_impl(target, ctx):
    emboss_info = target[EmbossInfo]
    src = emboss_info.direct_source
    lua_files = [ctx.actions.declare_file(src.basename + ".lua", sibling = src)]
    args = ctx.actions.args()
    args.add("--input-file")
    args.add_all(emboss_info.direct_ir)
    args.add("--output-file")
    args.add_all(lua_files)
    ctx.actions.run(
        executable = ctx.executable._emboss_lua_compiler,
        arguments = [args],
        inputs = emboss_info.direct_ir,
        outputs = lua_files,
        mnemonic = "EmbossLua",
    ...
    transitive_files = depset(
        direct = lua_files,
        transitive = [
            dep[EmbossLuaInfo].transitive_files
            for dep in ctx.rule.attr.deps
            if EmbossLuaInfo in dep
        ],
    )
    return [
        EmbossLuaInfo(
...
def _lua_emboss_library_impl(ctx):
    ...
    return [
        dep[EmbossInfo],
        dep[EmbossLuaInfo],
        DefaultInfo(files = dep[EmbossLuaInfo].files),
    ]
...
lua_emboss_library = rule(
    ...
    provides = [EmbossInfo, EmbossLuaInfo],
)
