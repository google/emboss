...
Windows-based software.  Hardware almost always appears to use ASCII (encoded as
one character per byte, with the high bit always clear).
...
could add a "byte width" parameter to the string types, but it does not seem
important for now.  This decision can be revisited later.
