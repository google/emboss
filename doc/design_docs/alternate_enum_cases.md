...
nature for resolving other use-cases. Either multiple names could be emitted
(those generated by `enum_case` as well as `name`), or under the principle of
"specific overrides general" a `name`-like attribute could override any
`enum_case` attribute. The specifics of this should be addressed in a design for
that feature. Additionally, the implementation of `enum_case` should be made to
allow `name` or a similar attribute to be added without major refactoring.
